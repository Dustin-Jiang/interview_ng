...
- `composables/`：函数式 ViewModel；`useAsync(loader)` 是所有请求的基础原语（`{data, loading, error, run}`）；`useBoardChannel` 是唯一模块级单例 + 引用计数，`useBoardRefresh(events, cb)` 300ms 防抖重拉是列表页实时刷新标准模式；**`useSystemStatus` 是系统状态（阶段 / 出价步长）的唯一共享数据源**（模块级单例 + 并发去重 + 写入方落库后强制刷新），捡漏页/候选人页/系统状态页一律消费它，禁止各自再拉一份；`useOidcSettings` 收编登录认证设置页的配置/角色名单/部门名单（两类规则的目标下拉）/保存/探测/清除密钥；`useWaitingQueue` 是候场队列（「已签到待分配」档）的唯一共享数据源（模块级单例 + 引用计数 + 请求序号：按看板通道事件防抖重拉，并丢弃并发重拉时先到的旧响应）——房间侧栏的拉取池与候场大屏的档内顺序因此同源同序，**禁止各自再拉一份**；`useTheme` 是深色模式的唯一数据源（模式 + 解析结果 + 落 `<html>.dark`）；**参考数据也各有唯一共享数据源**：`useDepartments`（部门名单）、`useRoles`（角色名单）、`useAdmissionList`（录取决定列表）、`useRooms`（房间列表 + `roomLabelOf` 按 id 反查展示名）——一律模块级单例，**谁都不许再自己 `departmentApi.list()` / `roleApi.list()` / `admissionApi.list()` / `roomApi.list()`**。它们与上面两个单例的差别是**不自作主张拉取**：参考数据的读取方各有门控（志愿弹窗仅在打开时、候选人页仅在 `candidates.browse_all` 时、房间列表页仅在列表态），由读取方按自己的门控调 `load()`，写入方（部门/角色增删改、录取表态）落库后立刻 `load()` 强制刷新；`useRooms` 另按消费者订阅房间**建/删/改名**三个事件（`domain/room.ts#ROOM_BOARD_EVENTS`），房间卡片上「当下是谁」那些变化只有房间列表页渲染，留在该页自己的订阅里。**读这些模块级单例的消费方若用 `watch` 决定「何时该拉」，必须带 `immediate: true`**——从别的页面切进来时单例往往已经就绪，等不到 `false→true` 的跳变，少了它就只有整页刷新才碰巧正常（`useAdmissions` 与系统状态页的预览都踩过这个坑）。
...
- **后端**
  ...
  - **消息通道只对「面试中」（`IN_PROGRESS`）开放**：结档档位不止 `COMPLETED`——把**已绑定房间**的候选人重置到 `ADMISSION_PENDING`/`ADMITTED`（面试完成之后的录取档）同样自动解绑房间并留档，否则会停在「录取档却仍占着房间、还能继续写记录」。判据是 `model.CandidateStatus.InProgress()`（**只认 `IN_PROGRESS`**；前端 `domain/status.ts#isInProgress` 同一判据，**改一处必须改另一处**）：`AppendMessage` 对「待面试」（`ASSIGNED`，已拉进房间但还没点开始）返回 `ErrInterviewNotStarted`（400），对结档房间返回 `ErrInterviewFinished`（400），空房间仍返回 `NotFound`；房间输入区据此禁用并给出对应占位文案。注意与 `Interviewing()`/`isInterviewing`（`ASSIGNED`|`IN_PROGRESS`，只描述「房间还绑着候选人」，用于结档解绑后的本地清理）区分开。
...
- **前端**
  ...
  - **房间展示名**：任何界面都不显示房间编号，统一走 `domain/room.ts#roomLabel(room)`——入参是**房间对象**（不是 name 字符串），未命名/非字符串 name → 「未命名」；手上是房间对象时直接调用，只有手上仅有候选人 `room_id` 的地方用 `composables/useRooms.ts` 的 `roomLabelOf`（房间列表的唯一共享数据源：`load()` 一次拉全 `GET /rooms?limit=200`，映射里存的就是已解析的展示名，房间建/删/改名由它自己订阅刷新）按 id 反查——目前只有**候选人详情（房间行 / 「进入房间」按钮）、候选人管理表（房间列）、候场大屏（房间列）**。**左侧名册（`RosterList`）一律不显示房间名**（按需求去掉；它的「按项 `#meta`」插槽因此已删除，名册行只有姓名 / 状态徽章 / 简介）。
  - **键盘**：键位放在**各页自己的 window 处理**里，但注册/注销与守卫收在 `useRosterHotkeys`（`useRosterSelection` 只提供状态与 `goPrev/goNext`，不注册全局键；名册也不做 ↑/↓ 列表导航）。两个名册页统一 **↑/↓ 切换候选人**；捡漏页另用 **←/→ 按步长调整报价**、**Enter 保存报价**——焦点在出价输入框内同样生效（该框只放数字、无光标需求，标记 `data-bid-input` 并作为 `isHotkeyInput` 传入），其它输入控件（搜索框等）与真按钮/链接一律让位（守卫 `lib/dom.ts#isEditableTarget` / `isActivatableElement`）。捡漏页的出价框是普通 `Input`（草稿即文本、输入即时同步），**不要换回 `ui/number-field`**：它自带 ↑/↓ 步进，会与新键位打架。键位提示统一用 `ui/kbd` 的 `Kbd` 键帽**直接标在对应控件上**（按钮文字后附 `↑`/`↓`/`Enter`/`Esc`，段式控件项内附序号 `1`/`2`/`3`，捡漏页的步进与保存按钮干脆以 `←`/`→`/`Enter` 键帽为内容），**不再写进 `title`**——原生 tooltip 触屏完全看不到；键帽落在 `bg-muted` 表面（如段式控件轨道）上时必须 `tone="surface"`，否则与底色同色而消失。
