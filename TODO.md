...
5. **预览**：全部映射结果交给**分页表格**直接展示（无「只看前 N 行 / 显示全部」切换，分页控件负责浏览），每行按目标字段展开（学号 / 姓名 / 个人简介）+ 校验状态（**不展示源表原始行**）；表达式输入 **300ms 防抖**，统计条（总 / 新建 / 更新 / 失败）挂在表格工具条。**表格不嵌套 Card**（与列表页一致，用 `DataTableSection`）。（最初设计为「原始行 JSON → 映射后 JSON → 校验状态」三段对照 + 左右分栏 + 前 10 行切换，后按反馈逐条调整。）
...
11. **表达式记忆**：映射表达式自动存 localStorage 的「最后状态」（下次进入预填，无需命名/管理预设）；**不保留导入历史**；**不提供模板下载**。（原设计为命名预设，后按反馈改为只记住最后一次取值。）
...
- [x] `composables/useCandidateImport.ts`（文件/解析/映射/预览/提交/表达式记忆）；解析与映射的纯函数放 `domain/` 便于复用。
- [x] 预览：映射结果按字段展开的**分页表格** + 校验状态 + 统计条（工具条）；提交后行级报告同样以分页表格就地展示（第 ③ 步的报告/被拒清单也不再套 Card）。
...
- **表达式记忆**：不做命名预设的保存/删除 UI，改为自动持久化「最后一次表达式」（`interview_ng_import_mapping`），进入页面即预填；「继续导入下一批」也保留表达式（仅清文件与结果）。旧键 `interview_ng_import_presets` 读取时清理。
...
- 边界（浏览器就地报错，文案自明）：2001 行 →「单次最多导入 2000 行（当前 2001 行）」；空表头 →「第 1 行存在空表头，请补全列名后重传」；6MB 文件 →「文件超过 5MB，请拆分后重传」；**非 `.xlsx`**（真实 CSV）→「只支持 .xlsx 工作簿」（SheetJS 会把 `.csv` 按二进制猜编码解析出乱码表头，必须在上传环节挡掉）。
...
- 学号列若在 Excel 中以数字存储且带前导零，会**静默丢失**（按用户指示不做特殊处理）——预览的「学号」列可肉眼核对（`123` vs `00123`）。
