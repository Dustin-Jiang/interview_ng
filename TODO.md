...
5. **预览**：逐行映射结果按目标字段展开成列（学号 / 姓名 / 个人简介）+ 校验状态（**不展示源表原始行**），默认前 10 行（可切"显示全部"），表达式输入 **300ms 防抖**，顶部统计条（总 / 新建 / 更新 / 失败）。（最初设计的三段对照含「原始行 JSON」与「映射后 JSON」两栏，后按反馈改为字段列表格。）
...
- [x] 预览：映射结果按字段展开的表格 + 校验状态 + 统计条 + 前 10 行/全部切换；提交后行级报告就地展示。
...
- **预览表形态**：按反馈去掉了「原始行 JSON」栏，映射结果由 JSON 串改为按目标字段展开的列（学号 / 姓名 / 个人简介），保留校验状态列；映射与预览也由左右分栏改为上下布局。
