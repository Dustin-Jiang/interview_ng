...
- **数据导入**（`/settings/imports`）：上传 `.xlsx` 工作簿 → 每行成为 JSON 对象（首行表头为 key）→ 用 JMESPath 逐字段映射成候选人 → 实时预览 → 单事务全或无落库。解析与映射全在浏览器（`xlsx` 官方 CDN tarball 的 mini 构建 + `@jmespath-community/jmespath`，仅此分区懒加载），服务端只收映射后的行；映射结果按 JSON 转义集解释字面量转义（`\n` 等），换行可正确显示。
- **面试房间**（`/rooms` / `/rooms/:roomId`）：房间列表（每间显示绑定候选人及其当前状态），点击进入具体房间查看实时消息与阶段控制；房间内基于 WebSocket 群聊。
