...
- **数据导入**（`/settings/imports`）：上传 `.xlsx` 工作簿 → 每行成为 JSON 对象（首行表头为 key）→ 用 JMESPath 逐字段映射成候选人 → 实时预览 → 单事务全或无落库。解析与映射全在浏览器（`xlsx` 官方 CDN tarball 的 mini 构建 + `@jmespath-community/jmespath`，仅此分区懒加载），服务端只收映射后的行。
