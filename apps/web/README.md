...
- **候选人管理**（`/`）：列表 / 创建 / 签到 / 分配房间 / 状态筛选，基于 REST。
...
| **ViewModel** | `src/composables` | `useXXX()` 组合式：持有 reactive 状态，把 Service 事件/回执转成可绑定状态，`onScopeDispose` 自管清理 | `useCandidates.ts`, `useRoomList.ts`, `useRoomChat.ts`, `useAsync.ts` |
...
- **组合式函数替代全局 store**：每个 `useXXX()` 在其调用者（如组件）各自的作用域内自管理状态，`onScopeDispose` 自动清理（如 `useRoomChat` 自动断开 WS），无全局单例与生命周期泄漏。
