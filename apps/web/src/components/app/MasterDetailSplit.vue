...
<template>
  ...
  <div class="mx-auto flex h-full w-full max-w-content overflow-hidden">
    <aside
      :class="cn(asideClass, 'h-full min-w-0 shrink-0 flex-col', props.asideWidth)"
      :aria-label="props.asideLabel"
    >
      <slot name="aside" />
    </aside>

    <section :class="cn(detailClass, 'h-full min-w-0 flex-1 flex-col')" :aria-label="props.detailLabel">
      <!-- 移动端返回栏 -->
